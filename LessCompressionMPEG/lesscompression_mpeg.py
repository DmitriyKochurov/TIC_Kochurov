...
def main(frame1, frame2, saveOutput=True, blockSize=16):
    bitsAnchor = []
    bitsDiff = []
    bitsPredicted = []
    h, w, ch = frame1.shape
    ...
    for i in range(0, 3):
        anchorFrame_c = frame1[:, :, i]
        targetFrame_c = frame2[:, :, i]
    ...
    if not isdir:
        os.mkdir(outfile)
    if saveOutput:
        cv2.imwrite(f"{outfile}/First frame.png", frame1)
        cv2.imwrite(f"{outfile}/Second frame.png", frame2)
